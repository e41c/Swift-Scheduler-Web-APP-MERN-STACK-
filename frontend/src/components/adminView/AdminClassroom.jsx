...
export default function AdminClassroom() {
  ...
  const onDelete = async (e) => {
    e.preventDefault();
    const deleteUrl = `/classrooms/${classroomID}`;
    try {
      // Validation
      if(classroomID === ''){
  ...
  const onRegister = async (e) => {
    ...
    try {
      ...
      // Post request to register a Classroom
  ...
  return (
    <div className='mt-24 h-[calc(100vh-4rem)] overflow-auto'>
      <h1 className='text-12xl text-white font-bold mb-2 uppercase text-center'>Delete/Register Classroom</h1>
      ...
      <div className="w-full max-w-lg mx-auto mt-8 bg-[#1d1d1d] shadow-md rounded px-8 pt-6 pb-8">
        ...
        <label className="block text-white text-sm font-bold mb-2">
          ...
          <input className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
            ...
            name='ClassroomID'
            value={classroomID}
            onChange={(e) => setClassroomID(e.target.value)}></input>
          <button type="submit" className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline ">
          Delete
        </button>
        </form>
